...
char *version = "Version 0.3.3";

int dd = 6; // -dd max allowed errors before ban
int matchMax = 3; // cross check max errors before ban
int banfindTest = 0;
...
void dupeTest(int kill) {
	...
	int tempmatch = matchMax;
	if (kill == 0) killitwithFire = 0; // disable the banning before running this!!
	else matchMax = kill; // set matchMax to int passed to function and allow bans!
	...
	matchMax = tempmatch; // restore origianl settings of match max
...
void creatban() {
	...
	printf("\nMatching auth.log with access.log...");
	fr = fopen(tempauth, "r");
	while(fgets(line, 256, fr) != NULL) {
	...
	printf("\nMatching errors and access logs...");
...
int main(int argc, char* argv[]) {
	...
	accesslog();
	authlog();
	if (strstr(argv[1], "-d")) dupeTest(0);
	if (strstr(argv[1], "-x")) dupeTest(dd); // -- -dd now -x
	creatban();
	removeTmp();
	return 0;
